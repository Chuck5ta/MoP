...
void WorldSession::SendNameQueryOpcode(Player* p)
{
    ...
    // guess size
    WorldPacket data(SMSG_QUERY_PLAYER_NAME_RESPONSE, (8 + 1 + 1 + 1 + 1 + 1 + 10));
...
void WorldSession::SendNameQueryOpcodeFromDBCallBack(QueryResult* result, uint32 accountId)
{
    ...
    // guess size
    WorldPacket data(SMSG_QUERY_PLAYER_NAME_RESPONSE, (8 + 1 + 1 + 1 + 1 + 1 + 1 + 10));
...
void WorldSession::HandleCorpseQueryOpcode(WorldPacket & /*recv_data*/)
{
    ...
    if (!corpse)
    {
        WorldPacket data(CMSG_CORPSE_QUERY, 1);
...
void WorldSession::HandleQueryQuestsCompletedOpcode(WorldPacket& /*recv_data */)
{
    ...
    WorldPacket data(SMSG_RAID_READY_CHECK_COMPLETED, 4 + 4 * count);
    data << uint32(count);
