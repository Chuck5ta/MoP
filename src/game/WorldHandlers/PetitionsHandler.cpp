...
{
    DEBUG_LOG("Received opcode CMSG_PETITION_RENAME");   // ok
    // recv_data.hexlike();
    ...
    WorldPacket data(CMSG_PETITION_RENAME, 8 + newname.size() + 1);
    data << petitionGuid;
    data << newname;
    SendPacket(&data);
}
...
{
    DEBUG_LOG("Received opcode CMSG_PETITION_DECLINE");  // ok
    // recv_data.hexlike();
    ...
    {
        WorldPacket data(CMSG_PETITION_DECLINE, 8);
