...
void WorldSession::HandleGuildDeclineOpcode(WorldPacket& recvPacket)
{
    ...
    if (Player* inviter = sObjectMgr.GetPlayer(GetPlayer()->GetGuildInviterGuid()))
        inviter->SendGuildDeclined(GetPlayer()->GetName(), recvPacket.GetOpcode() == CMSG_AUTO_DECLINE_GUILD_INVITES);
...
void WorldSession::HandleGuildInfoOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    WorldPacket data(CMSG_GUILD_INFO_TEXT, (guild->GetName().size() + 4 + 4 + 4));
    data << guild->GetName();
...
{
    DEBUG_LOG("WORLD: Received (SMSG_GUILD_PERMISSIONS_QUERY_RESULTS)");
...
{
    DEBUG_LOG("WORLD: Received CMSG_AUTO_DECLINE_GUILD_INVITES_TOGGLE");
