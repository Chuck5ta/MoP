...
void Unit::SendMeleeAttackStart(Unit* pVictim)
{
    ...
    SendMessageToSet(&data, true);
    DEBUG_LOG("WORLD: Sent SMSG_ATTACK_START");
}
...
{
    DEBUG_FILTER_LOG(LOG_FILTER_COMBAT, "WORLD: Sending SMSG_ATTACKER_STATE_UPDATE");
    ...
    uint32 count = 1;
    WorldPacket data(SMSG_ATTACKER_STATE_UPDATE, 16 + 45);    // we guess size
...
{
    // we guess size
    WorldPacket data(SMSG_SPELL_HEAL_LOG, (8 + 8 + 4 + 4 + 1));
...
void Unit::BuildForceMoveRootPacket(WorldPacket* data, bool apply, uint32 value)
{
    if (apply)
    {
        data->Initialize(SMSG_MOVE_ROOT, 13);
    ...
    {
        data->Initialize(SMSG_MOVE_UNROOT, 13);
