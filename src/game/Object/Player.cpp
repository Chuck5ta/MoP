...
void Player::SendInitialActionButtons() const
{
    ...
    WorldPacket data(SMSG_UPDATE_ACTION_BUTTONS, 1 + (MAX_ACTION_BUTTONS * 4));
...
void Player::SendLockActionButtons() const
{
    ...
    WorldPacket data(SMSG_UPDATE_ACTION_BUTTONS, 1);
...
{
    WorldPacket data(CMSG_CONFIRM_RESPEC_WIPE, (8 + 4));
    data << ObjectGuid(guid);
...
void Player::SendPetSkillWipeConfirm()
{
    ...
    if (!pet)
        return;
    WorldPacket data(SMSG_PET_REMOVED_SPELL, (8 + 4));
...
void Player::SendQuestUpdateAddCreatureOrGo(Quest const* pQuest, ObjectGuid guid, uint32 creatureOrGO_idx, uint32 count)
{
    ...
    WorldPacket data(SMSG_QUESTUPDATE_ADD_CREDIT, (4 * 4 + 8));
    DEBUG_LOG("WORLD: Sent SMSG_QUESTUPDATE_ADD_CREDIT");
    data << uint32(pQuest->GetQuestId());
    data << uint32(entry);
    data << uint32(count);
...
void Player::SendDungeonDifficulty(bool IsInGroup)
{
    ...
    WorldPacket data(CMSG_SET_DUNGEON_DIFFICULTY, 12);
...
{
    WorldPacket data(SMSG_UPDATE_CURRENCY, m_currencies.size() * 4);
...
void Player::ModifyCurrencyCount(uint32 id, int32 count, bool modifyWeek, bool modifySeason, bool ignoreMultipliers)
{
    ...
    if (newTotalCount != oldTotalCount)
    {
        ...
        if (IsInWorld() && !GetSession()->PlayerLoading())
        {
            ...
            WorldPacket packet(SMSG_SETUP_CURRENCY, 13);
...
void Player::SendPvPRewards()
{
    ...
    WorldPacket data(SMSG_REQUEST_PVP_REWARDS_RESPONSE, 6 * 4);
