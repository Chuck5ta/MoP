...
{
    WorldPacket data(SMSG_GUILD_BANK_QUERY_TEXT_RESULT, (4 * 2 * 8 + 4 * 3 + 1) * m_Ranks.size()); // we can only guess size
...
void Guild::DisplayGuildEventLog(WorldSession* session)
{
    // Sending result
    ByteBuffer buffer;
    WorldPacket data(SMSG_GUILD_EVENT_LOG_QUERY_RESULT, 0);
    // count, max count == 100
    ...
    session->SendPacket(&data);
    DEBUG_LOG("WORLD: Sent (SMSG_GUILD_EVENT_LOG_QUERY_RESULT)");
}
...
void Guild::SendGuildBankTabText(WorldSession* session, uint8 TabId)
{
    ...
    WorldPacket data(CMSG_GUILD_BANK_QUERY_TEXT, 1 + tab->Text.size() + 1);
